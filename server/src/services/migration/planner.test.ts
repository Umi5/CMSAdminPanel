...
import { planMigration, computeMigratedValues, type RefExistsInTarget } from './planner';
...
const current: Schema = {
  ...
  fields: [
    ...
    { id: 'producer', name: 'Producer', type: 'reference', required: false, referenceSchemaId: 'sch_producer' },
  ],
};

const entries: Entry[] = [
  { id: 'e1', schemaId: 'sch_wine', values: { name: 'A', year: '2015', producer: 'p1' }, createdAt: 't', updatedAt: 't' },
  { id: 'e2', schemaId: 'sch_wine', values: { name: 'B', year: 'vintage', producer: 'p2' }, createdAt: 't', updatedAt: 't' },
  { id: 'e3', schemaId: 'sch_wine', values: { name: 'C', year: 'n/a' }, createdAt: 't', updatedAt: 't' },
  { id: 'e4', schemaId: 'sch_wine', values: { name: 'D' }, createdAt: 't', updatedAt: 't' },
];
...
function change(plan: { changes: FieldChange[] }, fieldId: string, kind: string): FieldChange | undefined {
  return plan.changes.find((c) => c.fieldId === fieldId && c.changeKind === kind);
}

describe('planMigration', () => {
  it('treats a rename as safe with nothing to migrate', () => {
    const draft = draftWith(current.fields.map((f) => (f.id === 'year' ? { ...f, name: 'Vintage Year' } : f)));
  ...
  it('flags only the unconvertible values on the hard text->number retype', () => {
    const draft = draftWith(current.fields.map((f) => (f.id === 'year' ? { ...f, type: 'number' } : f)));
    const plan = planMigration(current, draft, entries, noRefs);
    const retype = change(plan, 'year', 'field_retyped');
    expect(retype?.severity).toBe('risky');
    expect(retype?.cleanCount).toBe(1); // "2015"
    expect(retype?.needsAttention.map((i) => i.entryId).sort()).toEqual(['e2', 'e3']); // vintage, n/a
  ...
  it('adding an optional field is safe; a required one needs a value in every entry', () => {
    const optional = planMigration(
      current,
      draftWith([...current.fields, { id: 'notes', name: 'Notes', type: 'text', required: false }]),
      entries,
      noRefs,
    );
    ...
    const required = planMigration(
      current,
      draftWith([...current.fields, { id: 'region', name: 'Region', type: 'text', required: true }]),
      entries,
      noRefs,
    );
  ...
  it('making a field required flags the entries that are empty', () => {
    const draft = draftWith(current.fields.map((f) => (f.id === 'year' ? { ...f, required: true } : f)));
    const plan = planMigration(current, draft, entries, noRefs);
  ...
  it('retargeting a reference flags values not present in the new target', () => {
    const draft = draftWith(
      current.fields.map((f) => (f.id === 'producer' ? { ...f, referenceSchemaId: 'sch_region' } : f)),
    );
    // p1 exists in the new target, p2 does not.
    const refExists: RefExistsInTarget = (schemaId, entryId) => schemaId === 'sch_region' && entryId === 'p1';
...
describe('computeMigratedValues', () => {
  const toNumberDraft = draftWith(current.fields.map((f) => (f.id === 'year' ? { ...f, type: 'number' } : f)));

  it('converts clean values and applies overrides for the rest', () => {
    const e2 = entries[1]!; // year: "vintage"
    const values = computeMigratedValues(current, toNumberDraft, e2, { year: 1990 }, noRefs);
  ...
  it('keeps a cleanly-converted value with no override', () => {
    ...
    const values = computeMigratedValues(current, toNumberDraft, e1, undefined, noRefs);
    expect(values.year).toBe(2015);
  });

  it('drops the value of a removed field', () => {
    const removedDraft = draftWith(current.fields.filter((f) => f.id !== 'year'));
    const values = computeMigratedValues(current, removedDraft, entries[0]!, undefined, noRefs);
