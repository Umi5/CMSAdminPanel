...
import { newDraftField, schemaToDraft, type SchemaDraftPayload } from '../hooks/useSchemaDraft';
...
export function SchemaBuilderPage() {
  ...
  useDocumentTitle(mode === 'create' ? 'New content type' : 'Edit content type');

  const existing = mode === 'edit' && schemaId ? getSchema(schemaId) : undefined;
  const backTo = mode === 'edit' && schemaId ? `/schemas/${schemaId}` : '/schemas';

  if (mode === 'edit') {
    if (loading && !existing) return <LoadingState label="Loading content type…" />;
    if (!existing) {
      return (
        <EmptyState
  ...
  const submit = async (draft: SchemaDraftPayload) => {
    if (mode === 'create') {
      ...
      try {
        ...
        showToast(err instanceof ApiError ? err.message : 'Failed to create content type', 'error');
      } finally {
        setSaving(false);
      }
      return;
    ...
    setSaving(true);
    try {
      const plan = await api.post<MigrationPlan>(`/schemas/${schemaId}/migration-plan`, { draft });
      ...
      showToast(err instanceof ApiError ? err.message : 'Failed to plan changes', 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  const applyMigration = async (overrides: MigrationOverrides) => {
    ...
    try {
      ...
      if (err instanceof ApiError && err.status === 409) {
        const conflict = err.details as { reason?: string; plan?: MigrationPlan } | undefined;
        if (conflict?.plan) {
  ...
  return (
    <Box>
      ...
      <PageHeader
        title={mode === 'create' ? 'New content type' : `Edit ${existing?.name ?? ''}`}
        subtitle={
          mode === 'create'
