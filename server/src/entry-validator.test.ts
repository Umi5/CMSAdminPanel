...
const schema: Schema = {
  ...
  fields: [
    ...
    { id: 'ref', name: 'Ref', type: 'reference', required: false, referenceSchemaId: 'other' },
  ],
};
...
describe('validateEntryValues', () => {
  it('accepts a well-formed entry', () => {
    expect(
      validateEntryValues(schema, { name: 'A', age: 3, active: true, dob: '2020-01-01' }),
    ).toEqual([]);
  });
  ...
  it('allows empty optional fields but flags wrong types', () => {
    const errors = validateEntryValues(schema, { name: 'A', age: 'not a number' });
    expect(errors.map((e) => e.fieldId)).toEqual(['age']);
  });

  it('treats 0 and false as present, not empty', () => {
    expect(validateEntryValues(schema, { name: 'A', age: 0, active: false })).toEqual([]);
  });

  it('rejects invalid date strings', () => {
    const errors = validateEntryValues(schema, { name: 'A', dob: 'not-a-date' });
    expect(errors.map((e) => e.fieldId)).toEqual(['dob']);
  });

  it('checks reference existence only when a resolver is supplied', () => {
    const refExists = (sid: string, eid: string): boolean => sid === 'other' && eid === 'exists';
    expect(validateEntryValues(schema, { name: 'A', ref: 'exists' }, refExists)).toEqual([]);
    const errors = validateEntryValues(schema, { name: 'A', ref: 'ghost' }, refExists);
