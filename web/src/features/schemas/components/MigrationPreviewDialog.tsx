...
import type { FieldChange, MigrationOverrides, MigrationPlan } from '@cms/shared';
...
export function MigrationPreviewDialog({
  ...
  const setFix = (entryId: string, fieldId: string, value: unknown) => {
    setValues((prev) => ({ ...prev, [entryId]: { ...prev[entryId], [fieldId]: value } }));
  };
  ...
  return (
    <Dialog open={open} onClose={saving ? undefined : onCancel} maxWidth="md" fullWidth>
      ...
      <DialogContent dividers>
        <Alert severity={plan.totalNeedsAttention > 0 ? 'warning' : 'info'} className="mb-4">
        ...
        <Box className="flex flex-col gap-3">
          {orderedChanges.map((change) => {
            ...
            return (
              <Paper key={`${change.fieldId}-${change.changeKind}`} variant="outlined" className="p-4">
                ...
                {change.needsAttention.length > 0 && field && (
                  <Box className="mt-3 flex flex-col gap-2">
                    {change.needsAttention.map((issue) => (
                      <Box key={issue.entryId} className="flex flex-wrap items-center gap-x-3 gap-y-1">
                        ...
                        <FixInput
                          ...
                          onChange={(value) => setFix(issue.entryId, change.fieldId, value)}
                        />
                      </Box>
                    ))}
                  </Box>
