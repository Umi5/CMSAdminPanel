...
export type RefResolver = (referenceSchemaId: string, entryId: string) => boolean;
...
function checkType(field: Field, value: unknown, refExists?: RefResolver): string | null {
  switch (field.type) {
    case 'text':
    ...
    case 'reference': {
      ...
      if (refExists && field.referenceSchemaId && !refExists(field.referenceSchemaId, value)) {
...
export function validateEntryValues(
  ...
  for (const field of schema.fields) {
    ...
    if (isEmpty(value)) {
      if (field.required) {
        errors.push({ fieldId: field.id, fieldName: field.name, message: 'is required' });
      }
      continue;
    }
    const typeError = checkType(field, value, refExists);
    if (typeError) {
      errors.push({ fieldId: field.id, fieldName: field.name, message: typeError });
    }
  }
  return errors;
}
