...
function NavItem({ to, label, onNavigate }: { to: string; label: string; onNavigate?: () => void }) {
  return (
    <NavLink to={to} onClick={onNavigate} style={{ textDecoration: 'none', color: 'inherit' }} end>
      {({ isActive }) => (
        <ListItemButton selected={isActive} aria-current={isActive ? 'page' : undefined} sx={{ mb: 0.5 }}>
  ...
  return (
    <Box className="flex flex-col h-full">
      <Box className="flex items-center gap-2 px-5 py-5">
        <Box
          sx={{ width: 30, height: 30, borderRadius: 2, bgcolor: 'primary.main' }}
      ...
      <Box className="px-3 py-3">
        <List disablePadding>
          <NavItem to="/schemas" label="Content types" onNavigate={onNavigate} />
        </List>
      </Box>
      <Divider />
      ...
      <Box className="flex-1 overflow-y-auto px-3 pb-2">
        <List dense disablePadding>
          {schemas.map((schema) => {
            const active = location.pathname.startsWith(`/schemas/${schema.id}`);
            return (
            <ListItemButton
              key={schema.id}
              selected={active}
              ...
              sx={{ mb: 0.25 }}
            >
              <ListItemText primary={schema.name} primaryTypographyProps={{ fontSize: 14, fontWeight: 600, noWrap: true }} />
              <Chip size="small" variant="outlined" label={schema.fields.length} />
            </ListItemButton>
            );
          })}
        </List>
